...
class TemplateDay(BaseModel):
    ...
    items: list[TemplateItem] = Field(default_factory=list, max_length=100)
...
def _public_week(row):
    result = {key: row[key] for key in (
        "id", "week_key", "week_start", "week_end", "template_version",
        "snapshot_at", "synced_at", "days", "created_at", "updated_at",
    )}
    return jsonable_encoder(result)


def _template(cursor, user_id: UUID, *, lock: bool = False):
...
def _week(cursor, user_id: UUID, week_id: UUID, *, lock: bool = False):
    query = "SELECT * FROM training_weeks WHERE id = %s AND user_id = %s"
    if lock:
        query += " FOR UPDATE"
...
def _create_or_get_current(cursor, user_id: UUID):
    week_key, start, end = _current_week()
    template = _template(cursor, user_id, lock=True)
    cursor.execute(
        "SELECT * FROM training_weeks WHERE user_id = %s AND week_key = %s FOR UPDATE",
        (user_id, week_key),
    )
    row = cursor.fetchone()
    if row is not None:
    ...
    cursor.execute(
        """INSERT INTO training_weeks
           (user_id, week_key, week_start, week_end, template_version,
            snapshot_at, days, created_at, updated_at)
           VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
           ON CONFLICT (user_id, week_key) DO NOTHING RETURNING *""",
        (user_id, week_key, start, end, template["version"], now,
         Jsonb(_snapshot_days(template["days"], start)), now, now),
    )
    row = cursor.fetchone()
    if row is None:
        cursor.execute(
...
def _all_sets(days: list[dict]):
    for day in days:
        for item in day["items"]:
            for set_detail in item.get("sets", []):
...
def get_template(user_id: User):
    with connection() as conn, conn.cursor() as cursor:
        return _public_template(_template(cursor, user_id))
...
def get_week(week_id: UUID, user_id: User):
    with connection() as conn, conn.cursor() as cursor:
...
def sync_current_week(user_id: User):
    with connection() as conn, conn.cursor() as cursor:
        ...
        if created:
            return {"week": _public_week(week), "created": True, "synced": False}

        template = _template(cursor, user_id, lock=True)
        old_by_id = {
            item["item_id"]: deepcopy(item)
            for day in week["days"] for item in day["items"]
        }
        ...
        for day in merged:
            for item in day["items"]:
                ...
                if old is not None:
                    item["sets"] = old.get("sets", [])
                    item["completed_sets"] = len(item["sets"])
        ...
        for old_day in week["days"]:
            ...
            for old in old_day["items"]:
                if old["item_id"] not in current_ids and old.get("sets"):
                    retained = deepcopy(old)
                    retained["completed_sets"] = len(retained["sets"])
                    retained["removed_from_template"] = True
                    target_day["items"].append(retained)
...
def create_set(week_id: UUID, item_id: str, body: CreateSet, user_id: User):
    with connection() as conn, conn.cursor() as cursor:
...
def patch_set(
    week_id: UUID, item_id: str, request_id: str, body: PatchSet, user_id: User
):
    with connection() as conn, conn.cursor() as cursor:
        week = _week(cursor, user_id, week_id, lock=True)
        days = deepcopy(week["days"])
